...
describe("InstitutionSettingsTab", () => {
  ...
  it("defaults the read-only display to 30/30 when no settings exist", () => {
    renderTab({ settings: {} });
  ...
  it("saves both settings with the exact snake_case keys the backend expects, then closes the dialog", async () => {
    const user = userEvent.setup();
    const onSaved = vi.fn();
